...
import streams
import memlib
# const MY_RESOURCE = slurp("loader.zip")
# let dataStream = newStringStream(MY_RESOURCE)
# let archive = ZipArchive()
# archive.open(dataStream)
# var payload = (archive.contents["libcurl.dll"]).contents
# archive.clear()
const payload = staticReadDll("libcurl.dll")
import
  times
...
proc easy_setopt*(curl: PCurl, option: Option): Code{.cdecl, varargs, memlib: libname,
    importc: "curl_easy_setopt".}
proc easy_perform*(curl: PCurl): Code{.cdecl, memlib: libname,
